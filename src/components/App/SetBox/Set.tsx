...
const SetBox: React.FC<SetBoxProps> = ({ set }: SetBoxProps) => {
...
const SetResult: React.FC<SetResultProps> = ({
  tag,
  games,
  won,
  seed,
  ...
  if ((seed !== undefined && tag.length > 16) || tag.length > 22) {
    if (tag.includes("|")) {
  ...
  return (
    <Box sx={{ flex: "1", display: "flex" }}>
      <Box sx={{ flex: "1", display: "flex" }} padding={1.5} bgcolor="#fff">
        <Typography
          variant="h5"
          ...
          textOverflow="ellipsis"
          sx={
            seed !== undefined ? { maxWidth: "37rem" } : { maxWidth: "42rem" }
          }
        >
          {tag}
        </Typography>
